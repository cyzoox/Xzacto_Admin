...
const TransferLogs = ({navigation}) => {
  const {transfer_logs, getTransferLogs} = useStore();
  ...
  useEffect(() => {
    ...
    getTransferLogs(today, 1)
  },[]);

  const calculateTotal = () => {
    let total = 0;

    transfer_logs.forEach(item => {
    ...
    let lastdays = `${moment.unix(date).subtract(30, 'day').startOf('day')/ 1000}`;
      ...
      switch (filter) {
        ...
        case "Yesterday":
          ...
          getTransferLogs(yesterday,  1)
          break;
  
        case "This Week":
          getTransferLogs( `${thisweek+'-'+thisyear}`,  3)
          break;
  
        case "Last Week":
          getTransferLogs(`${lastweek+'-'+thisyear}`, 3)
          break;
  
        case "This Month":
          getTransferLogs(`${thismonth+'-'+thisyear}`,  2)
          break;
  
        case "Last Month":
          getTransferLogs(`${lastmonth+'-'+thisyear}`, 2)
          break;
  
        case "This Year":
          getTransferLogs(thisyear,  5)
          break;
  
        case "Last Year":
          getTransferLogs( lastyear,  5)
          break;
  
        default:
          getTransferLogs(filter, 1)
          break;
    ...
    <TouchableOpacity style={styles.listStyle} >
      <View style={{flexDirection: 'row'}}>
        ...
        <View>
          ...
          <Text style={{color: colors.black, marginBottom: 3, fontWeight:'400', fontSize: 15, fontFamily:'Monospace'}}>
            {item.quantity} {item.unit}/s
          </Text>
          <Text style={{color: colors.black, marginBottom: 3, fontWeight:'400', fontSize: 15, fontFamily:'Monospace'}}>
            Transferred to {item.store_name}
          </Text>
      ...
      <Text style={{fontSize: 17, color: colors.black, fontWeight: '700'}}>{formatMoney(item.quantity*item.sprice, { symbol: "₱", precision: 1 })}</Text>
