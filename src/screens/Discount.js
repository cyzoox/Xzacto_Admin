        ...
        let lastdays = `${moment.unix(date).subtract(30, 'day').startOf('day')/ 1000}`;
          ...
          switch (filter) {
            ...
            case "Yesterday":
                ...
                getDiscount(yesterday,  1, STORE._id)
              break;
      
            case "This Week":
                getDiscount( `${thisweek+'-'+thisyear}`,  3, STORE._id)
              break;
      
            case "Last Week":
                getDiscount(`${lastweek+'-'+thisyear}`, 3, STORE._id)
              break;
      
            case "This Month":
                getDiscount(`${thismonth+'-'+thisyear}`,  2, STORE._id)
              break;
      
            case "Last Month":
                getDiscount(`${lastmonth+'-'+thisyear}`, 2, STORE._id)
              break;
      
            case "This Year":
                getDiscount(thisyear,  5, STORE._id)
              break;
      
            case "Last Year":
                getDiscount( lastyear,  5, STORE._id)
              break;
      
            default:
                getDiscount(filter, 1, STORE._id)
              break;
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <View>
              <Text style={{color: colors.black, marginBottom: 3, fontWeight:'700', fontSize: 16, fontFamily:'Monospace'}}>
                {`${item.transaction_id}`.replace(/(\d{3})(\d{3})(\d{4})/, "$1-$2-$3")}
              </Text>
